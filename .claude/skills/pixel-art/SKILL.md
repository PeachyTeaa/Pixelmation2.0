...
## Сначала посмотри на примеры

Если в `examples/` что-то лежит — сначала разбери оттуда ближайший по смыслу
файл и держи его стиль: палитру, размер холста, число кадров, шаг движения.

```bash
ls examples
npm run pxl -- info examples/run.pxlma
npm run pxl -- show examples/run.pxlma --slide 1 --rulers
```
