...
const HOTKEYS: Array<[string, string]> = [
  ...
  ['Ctrl+S', 'сохранить поверх того же файла'],
  ['Ctrl+Shift+S', 'сохранить как — выбрать новый файл'],
    ...
    <Modal open={open} title="Как это работает" onClose={onClose}>
      <section style={{ display: 'grid', gap: '0.75rem' }}>
        ...
        <p>
          ...
          подправить её и вернуться к кадрам. Там же кнопка <strong>«Загрузить текстуру…»</strong>
          подставляет вместо текущей текстуру из другого файла — кадры остаются на месте и просто
          перекрашиваются.
        </p>
        <p>
        ...
        <p>
          <strong>Сохранение.</strong> Первый <kbd>Ctrl+S</kbd> спрашивает, куда положить файл, а
          дальше пишет прямо в него — копии в «Загрузках» не плодятся. Открытый или перетащенный
          файл сразу становится целью. <kbd>Ctrl+Shift+S</kbd> и кнопка «Сохранить как…» делают
          новый файл и переключаются на него. Firefox и Safari перезаписывать не умеют — там
          по-прежнему скачивается копия, о чём приложение честно предупредит.
        </p>
      </section>
